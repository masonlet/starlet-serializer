...
namespace Starlet::Serializer {
  bool Writer::writeScene(const Starlet::Scene::Scene& scene) {
  ...
  bool Writer::writeCameras(std::ostream& file, const Starlet::Scene::Scene& scene) {
    file << "comment, name, pos(xyz), rot(yaw pitch), fov, nearPlane farPlane, camSpeed\n";
    for (const auto& [entity, cam] : scene.getEntitiesOfType<Scene::Camera>()) {
  ...
  bool Writer::writeModels(std::ostream& file, const Starlet::Scene::Scene& scene) {
    file << "\ncomment, name, meshPath, pos(xyz), rot(xyz), scale(xyz), colour(Int, Named Colour, Random, Rainbow, PLY), specular(rgb, power)\n";
    for (const auto& [entity, model] : scene.getEntitiesOfType<Starlet::Scene::Model>()) {
      ...
      Starlet::Scene::ColourComponent colour{};
      if (scene.hasComponent<Starlet::Scene::ColourComponent>(entity))
        colour = scene.getComponent<Starlet::Scene::ColourComponent>(entity);
  ...
  bool Writer::writeLights(std::ostream& file, const Starlet::Scene::Scene& scene) {
    file << "\ncomment, name, type, pos (xyz), diffuse (rgba), attention (xyzw), direction, param1 (spotlight inner, spotlight outer), param2 (on/off)\n";
    for (const auto& [entity, light] : scene.getEntitiesOfType<Starlet::Scene::Light>()) {
      if (!scene.hasComponent<Scene::TransformComponent>(entity)) continue;

      const Starlet::Scene::TransformComponent& transform = scene.getComponent<Starlet::Scene::TransformComponent>(entity);
      const Starlet::Scene::ColourComponent& colour = scene.getComponent<Starlet::Scene::ColourComponent>(entity);
  ...
  bool Writer::writeTransform(std::ostream& file, const Scene::TransformComponent& transform, bool includeSize) {
    file << transform.pos << ", ";
    file << transform.rot << ", ";
    if (includeSize) file << transform.size << ", ";
    return !file.fail();
  }
  bool Writer::writeColourMode(std::ostream& file, const Scene::Model& model, const Starlet::Scene::ColourComponent& colour) {
    switch (model.mode) {
    case Starlet::Scene::ColourMode::Solid: {
      Math::Vec4 rgba{ colour.colour.x * 255.0f, colour.colour.y * 255.0f, colour.colour.z * 255.0f, colour.colour.w * 255.0f };
    ...
    case Starlet::Scene::ColourMode::Random:           file << "Random"; break;
    case Starlet::Scene::ColourMode::VerticalGradient: file << "Rainbow"; break;
    case Starlet::Scene::ColourMode::PLYColour:        file << "PLY"; break;
    default: break;
    }
