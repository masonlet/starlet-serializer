...
namespace Starlet::Serializer {
	bool BmpParser::parse(const std::string& path, BmpData& out) {
		...
		if (!loadBinaryFile(file, path))
			return Logger::error("BMPParser", "parseBMP", std::string("Failed to load file: ") + path);

		if (file.empty()) return Logger::error("BmpParser", "parseBMP", "Input pointer is null\n");
		...
		if (needed > file.size())
			return Logger::error("BMPParser", "parseBMP", "File too small for declared dimensions: " + std::to_string(file.size()) + " bytes, need " + std::to_string(needed) + " bytes");
		...
		if (out.pixels.empty())
			return Logger::error("BMPParser", "parseBMP", "Failed to allocate memory for pixel data");
		...
		for (int row = 0; row < height; ++row) {
			...
			for (int col = 0; col < width; ++col) {
				...
				else return Logger::error("BMPParser", "parseBMP", "Buffer overrun detected while writing to 'dst'");
			}
		}
	...
	bool BmpParser::parseBmpHeader(const unsigned char* p, size_t fileSize, int32_t& height, int32_t& width, uint32_t& dataOffset) {
		if (!p) return Logger::error("BMPParser", "readHeader", "Null buffer");
		if (fileSize < 54) return Logger::error("BMPParser", "parseBMP", "File too small: " + std::to_string(fileSize) + " bytes");
		if (p[0] != 'B' || p[1] != 'M') return Logger::error("BMPParser", "readHeader", "Bad signature (not BM)");

		dataOffset = (uint32_t)p[10] | ((uint32_t)p[11] << 8) | ((uint32_t)p[12] << 16) | ((uint32_t)p[13] << 24);
		if (dataOffset >= fileSize) return Logger::error("BMPParser", "readHeader", "Bad data offset");
		...
		if (dibSize < 40) return Logger::error("BMPParser", "readHeader", "Unsupported DIB header size: " + std::to_string(dibSize));
		...
		if (width <= 0) return Logger::error("BMPParser", "readHeader", "Invalid width: " + std::to_string(width));
		...
		if (height == 0) return Logger::error("BMPParser", "readHeader", "Invalid height: " + std::to_string(height));

		uint16_t planes = static_cast<uint16_t>(static_cast<uint32_t>(p[26]) | (static_cast<uint32_t>(p[27]) << 8));
		if (planes != 1) return Logger::error("BMPParser", "readHeader", "Planes != 1: " + std::to_string(planes));

		uint16_t bpp = static_cast<uint16_t>(static_cast<uint32_t>(p[28]) | (static_cast<uint32_t>(p[29]) << 8));
		if (bpp != 24) return Logger::error("BMPParser", "readHeader", "Only 24bpp supported: " + std::to_string(bpp));
		...
		if (compression != 0) return Logger::error("BMPParser", "readHeader", "Compressed BMP not supported: " + std::to_string(compression));
		return true;
	}
}
